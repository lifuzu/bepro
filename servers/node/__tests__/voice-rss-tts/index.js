...
function _request(settings) {
  const req = _buildRequest(settings);
  ...
  request({
    ...
    if (!error && response.statusCode == 200) {
      if (settings.callback) {
        if (error) { settings.callback(error, null); } else if (body.indexOf('ERROR') == 0) { settings.callback(body, null); } else { settings.callback(null, body); }
      }
    }
  });
}
